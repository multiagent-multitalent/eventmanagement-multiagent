# Dashboard: AI Transparency Days 2026

*Zuletzt aktualisiert: 2026-03-31*

> ⚠️ **Projektabschluss: 30. Juni 2026** – Alle Planungsarbeiten, Verträge und die Übergabe an das COAI-Team müssen bis 30.06.2026 abgeschlossen sein.

---

## Gesamtstatus

| Aspekt | Status |
|---|---|
| Phase | Phase 1: Initialisierung → Übergang zu Phase 2 (Planung) |
| Nächster Meilenstein | Kick-off (01. April 2026) |
| Kritische Blocker | Venue-Buchung ausstehend, Team-Rollen zu besetzen |
| Team vollständig | ❌ Nein – Rollen noch zu besetzen (siehe `config/team.yaml`) |
| Planungsartefakte | ✅ Generiert – bereit zur Bestätigung (siehe `CONFIRM.md`) |

---

## Workstream-Status

| Workstream | Status | Agent | Artefakte | Nächster Schritt |
|---|---|---|---|---|
| Programm | 🟡 Konzept bereit | Programm | track-konzept.md, speaker-konzept.md, agenda-entwurf.md | Speaker-Einladungen (April) |
| Kommunikation | 🟡 Konzept bereit | Kommunikation | kommunikationsplan.md, social-media-kit.md | Social-Media-Accounts anlegen |
| Teilnehmer | 🟡 Konzept bereit | Operations | registrierung-konzept.md | Registrierungstool auswählen |
| Venue und Logistik | 🟡 Recherche bereit | Operations | venue-recherche.md | Anfragen bei Top-3-Venues |
| Catering | 🟡 Konzept bereit | Operations | catering-konzept.md | Caterer anfragen (nach Venue) |
| Technik | 🟡 Anforderungen bereit | Operations | technik-anforderungen.md | Technik-Check mit Venue |
| Personal | 🟡 Planung bereit | Koordination | helfer-planung.md | Helfer-Ausschreibung (Juli) |
| Sponsoring | 🟡 Konzept bereit | Koordination | sponsoring-konzept.md, budget-rahmen.md | Sponsoring-Prospekt erstellen |
| Unterkunft/Anreise | 🟡 Recherche bereit | Kommunikation | hotel-recherche.md | Hotel-Kontingente anfragen |
| Nachbereitung | ⚪ Noch nicht relevant | Dokumentation | – | – |

**Legende:** 🟢 Im Plan / abgeschlossen | 🟡 In Bearbeitung / Konzept bereit | 🔴 Kritisch / Blockiert | ⚪ Noch nicht relevant

---

## Generierte Artefakte (Übersicht)

### Programm
- ✅ `workstreams/programm/track-konzept.md` – 3 Tracks mit Themen, Formaten, Speaker-Profilen
- ✅ `workstreams/programm/agenda-entwurf.md` – Tag-für-Tag-Agenda (3 Tage)

### Kommunikation
- ✅ `workstreams/kommunikation/kommunikationsplan.md` – Kommunikationsplan mit Zeitlinie
- ✅ `workstreams/kommunikation/social-media-kit.md` – Posts für alle Phasen

### Venue & Logistik
- ✅ `workstreams/venue-logistik/venue-recherche.md` – 5 Venue-Vorschläge mit Bewertungsmatrix

### Teilnehmer
- ✅ `workstreams/teilnehmer/registrierung-konzept.md` – Ticket-Kategorien, Formular, Prozess

### Catering
- ✅ `workstreams/catering/catering-konzept.md` – Verpflegungsplan für 3 Tage, Budget-Kalkulation

### Technik
- ✅ `workstreams/technik/technik-anforderungen.md` – AV-Anforderungen pro Raum, WLAN-Specs

### Personal
- ✅ `workstreams/personal/helfer-planung.md` – 16 Helfer, Rollen, Schichtplan

### Sponsoring & Budget
- ✅ `workstreams/sponsoring/sponsoring-konzept.md` – Pakete und Sponsor-Recherche
- ✅ `workstreams/sponsoring/budget-rahmen.md` – Budget 30.000–65.000 €, Einnahmen-Planung

### Unterkunft & Anreise
- ✅ `workstreams/unterkunft-anreise/hotel-recherche.md` – Hotel-Empfehlungen und Kontingent-Plan

---

## Meilensteine

| Meilenstein | Zieldatum | Status |
|---|---|---|
| Kick-off | 01.04.2026 | 🟡 Vorbereitung läuft |
| Speaker-Einladungen versandt | 30.04.2026 | 🔴 Ausstehend |
| Registrierung geöffnet | 01.05.2026 | 🔴 Tool noch nicht ausgewählt |
| Venue gebucht | Mai 2026 | 🔴 Anfragen ausstehend (`venue-recherche.md`) |
| Zwischenbericht | 31.05.2026 | 🔴 Ausstehend |
| Speaker-Zusagen eingegangen | 15.06.2026 | ⚪ Noch nicht relevant |
| 🏁 **PROJEKTABSCHLUSS & Abschlussbericht** | **30.06.2026** | ⚪ Ausstehend |
| Programm finalisiert *(COAI)* | 31.07.2026 | ⚪ Post-Projekt |
| Event *(COAI)* | 14.–16.10.2026 | ⚪ Post-Projekt |
| Nachbereitung abgeschlossen *(COAI)* | 30.11.2026 | ⚪ Post-Projekt |

---

## Aktuelle Risiken

| Risiko | Wahrscheinlichkeit | Impact | Maßnahme | Status |
|---|---|---|---|---|
| Venue-Verfügbarkeit im Oktober (Messe-Saison!) | Hoch | Hoch | Sofort anfragen – Oktober ist Hochsaison | 🔴 Kritisch |
| Team-Rollen noch nicht besetzt | Hoch | Mittel | Im Kick-off (01.04.) klären | 🟡 Geplant |
| Budget-Freigabe ausstehend | Mittel | Hoch | Budget-Rahmen bestätigen lassen | 🟡 Konzept bereit |
| Website und Registrierungstool fehlen noch | Mittel | Mittel | Bis April umsetzen | 🟡 Konzept bereit |

---

## Offene Entscheidungen (Menschliche Bestätigung benötigt)

- [ ] Event-Datum 14.–16. Oktober 2026 bestätigen
- [ ] Venue-Auswahl treffen (nach Besichtigungen)
- [ ] Budget-Rahmen genehmigen (`workstreams/sponsoring/budget-rahmen.md`)
- [ ] Track-Konzept bestätigen (`workstreams/programm/track-konzept.md`)
- [ ] Speaker-Shortlist freigeben und Einladungen versenden
- [ ] Registrierungstool auswählen
- [ ] Ticketpreise final festlegen
- [ ] Streaming: ja oder nein?

→ Alle Entscheidungen zentral in `CONFIRM.md` aufgelistet.

---

## Projektzeitplan

**→ Detaillierter Zeitplan mit Wochen, Aufgaben und Meilensteinen:** `dashboard/zeitplan.md`

---

## Nächste Aktionen (priorisiert)

| Priorität | Aktion | Zuständig | Bis wann |
|---|---|---|---|
| 🔴 Sofort | Venue-Anfragen versenden (Oktober = Hochsaison!) | Operations | April 2026 |
| 🔴 Sofort | Team-Rollen besetzen (`config/team.yaml`) | Projektleitung | Kick-off |
| 🔴 Sofort | Keynote-Speaker anfragen (mind. 6 Monate vor Event!) | Programm | April 2026 |
| 🟡 Diese Woche | `CONFIRM.md` durchgehen und bestätigen | Projektleitung | April 2026 |
| 🟡 April | Registrierungstool einrichten und Website aufsetzen | Operations | 30.04.2026 |
| 🟡 April | Sponsoring-Ansprache starten | Koordination | Mai 2026 |
| 🟡 Juni | Übergabedokumentation für COAI-Team vorbereiten | Dokumentation | **30.06.2026** |
