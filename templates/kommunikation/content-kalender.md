# Content-Kalender-Vorlage

## Kommunikationsplan: AI Transparency Days 2026

---

## Phasen der Kommunikation

| Phase | Zeitraum | Hauptbotschaft |
|---|---|---|
| Teaser | April 2026 | „Save the Date" |
| Ankündigung | April 2026 | Event-Launch, Speaker-Ankündigung |
| Speaker-Phase | April–Juni 2026 | Programm-Highlights, Registrierungen gewinnen |
| Registrierung | Mai–September 2026 | Tickets kaufen |
| Countdown | September–Oktober 2026 | Vorfreude, letzte Infos |
| Live | Oktober 2026 | Live-Updates |
| Nachbereitung | November 2026 | Dankeschön, Materialien |

---

## Content-Kalender (Vorlage)

| Datum | Kanal | Typ | Inhalt (Stichworte) | Verantwortlich | Status |
|---|---|---|---|---|---|
| [Datum] | LinkedIn | Post | Save the Date | TBD | ⚪ Offen |
| [Datum] | Twitter/X | Post | Save the Date | TBD | ⚪ Offen |
| [Datum] | Newsletter | E-Mail | Event-Ankündigung | TBD | ⚪ Offen |
| [Datum] | Presse | Pressemitteilung | Event-Launch | TBD | ⚪ Offen |
| [Datum] | Website | Update | Programm veröffentlicht | TBD | ⚪ Offen |

**Status:** ⚪ Offen | 🟡 In Arbeit | 🟢 Veröffentlicht

---

## Zielgruppen und Kanäle

| Zielgruppe | Primäre Kanäle |
|---|---|
| Forschende | LinkedIn, Mailinglisten (z. B. ML-News), Konferenz-Communities |
| Praktiker | LinkedIn, Twitter/X |
| Studierende | Instagram, LinkedIn, Uni-Mailinglisten |
| Policymaker | LinkedIn, direkte Ansprache |
| Presse | Pressemitteilungen, E-Mail |

---

*Dieses Dokument regelmäßig aktualisieren. Neue Zeilen oben einfügen (neueste zuerst).*
